...
**Relative efficiency correction of LIGHTIGO spectra** (`data/efficiency_correction.py`, a port of the R procedure `external_data/Data/20240910_REC_FireFly.R`): `RelativeEfficiencyCorrection.from_lamp_files(deuterium_h5, halogen_h5)` derives the multiplicative factor `certified lamp irradiance / measured lamp spectrum` from a deuterium measurement (UV, 180–400 nm, LOWESS-smoothed as in R) and a halogen measurement (VIS, 350–900 nm, raw), joins them in the 350–400 nm overlap by a linear fit of one curve onto the other (R's `lm`), and exposes `rec(wavelength)` (factor, NaN outside the calibrated range), `rec.apply(spectra, wavelength, fill=...)`, `rec.sensitivity(...)` (= 1/factor) and `save`/`load` (`.npz`); `correct_spectra(...)` is the one-call form. Deviations from R, all switchable: smoothing and dead-pixel filling run per spectrometer channel (`channel_edges_nm`, FireFly gain steps at 260/370/460/690 nm plus the automatically detected axis reversals at 370 and 460 nm), the overlap is cross-faded instead of interleaved, and the halogen spectrum may be smoothed too. `scripts/build_efficiency_correction.py` writes the curve (`.npz`, `.tsv`) and a diagnostics figure to `Outputs/`; `scripts/calibrate_generator_lightigo.py --efficiency_correction D2.h5 HAL.h5` applies it before fitting, which is the physics cross-check of the lamp curve (the fitted response polynomial should come out flat). `tests/test_efficiency_correction.py` recovers a known sensitivity, including gain steps, from synthetic lamp spectra. The 2024-09-06 lamp files have only ~100 counts in the three channels below 460 nm, so their correction is unreliable there (see the calibration cross-check); re-measure the lamps with longer exposure and a dark frame.

**Calibration against LIGHTIGO HDF5 data** (`scripts/calibrate_generator_lightigo.py`, first used on native bismuth, `external_data/Data/matrix_01.h5`): builds a reference spectrum from the element-rich shots of a map (rolling-minimum baseline, median), fits the instrument FWHM on isolated lines, then fits the two-zone parameters (`Te1`, `Ne1`, `Te2/Te1`, `l_inner`, `l_outer`, `gamma_stark`, FWHM scale) by random search plus Nelder–Mead on the rms of `log10(area_syn/area_meas)` over the element's DB lines **after removing a smooth spectral response** `a + b·x + c·x²` (+ channel offset, `x = (λ − 300)/100`) fitted by least squares in every evaluation, plus a peak-normalised profile term. The response term is essential: uncalibrated LIGHTIGO spectra are ~100× less sensitive at 195 nm than at 470 nm. The same fit with `l_outer = 0` quantifies the shell's benefit, and parameters ending at the search-box edge are reported as unconstrained. Composition comes from the sample-matrix row named by `--mineral`, or `--composition` when the row is absent. Results go to `Outputs/calibrate_lightigo_<element>_<ts>/` (JSON, overlays, line-ratio plots, reference spectrum); the script never edits configs.

**Timed end-to-end smoke run** (`scripts/run_minerals_pipeline.py`): generation → `train_pretrain.py` → `train_finetune.py --task classification` as three subprocesses; records wall time, CPU time and utilisation (CPU/wall, the parallel headroom of a step), per-epoch times parsed from the Lightning progress bars, sizes (shots, pixels, splits, epochs, batch) and the held-out test metrics into `Outputs/pipeline_<experiment>_<ts>/timing.{json,md}`, and appends one row per step to `Outputs/pipeline_timings.csv` so optimisations can be compared across runs. `--skip_generate` and `--pretrain_run_dir` re-run later steps only.
...
| `data.n_classes` (classification) | `downstream.class_label: cluster` (default) → `downstream.n_clusters` KMeans clusters of the concentration vectors; `class_label: sample_type` → one class per row of the sample matrix (mineral identity), `n_classes` = number of distinct `sample_type_id` | `train_finetune.py` only |
...
| `libs_data_minerals_smoke.yaml` | physics v2 (`plasma_model: two_zone`, `fine_step_nm: 0.005`, 6 workers), sample matrix `REE_minerals_oxides.xlsx`, DB `LIBS_data.db`, **LIGHTIGO axis** (`wavelength_json: external_data/Data/matrix_01.h5`; `load_wavelength` reads `measurements/<first>/libs/calibration` from any `*.h5`), ranges calibrated on the Bi/hematite maps (`te1` 6–14 kK, `ne1` 1e16–1e18, `l_inner_cm` 0.01–0.5, `gamma_stark_nm` 0.002–0.01, FWHM 0.15 nm), `class_label: sample_type` | 8 (first rows) | 12 | 8 (minerals) | 50 | `random` |
